...
-- 明示的に型宣言を与える
-- “これはHaskellのプログラムを書く上で、（とても短い関数を書く場合は除いて）一般的に良い習慣だと考えられています”
...
-- “空のタプル()も型だということに注意してください。この型はただ1つの値()のみを持ちます”
...
--  * Show
--  * Read
-- Reed は結果単体の場合は型推論が効かないため、型注釈を用いる
readExample = read "5" :: Int
readExample' = read "[1, 2, 3, 4]" :: [Int]
